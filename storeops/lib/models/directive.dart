class Directive {
  ...
  factory Directive.fromJson(Map<String, dynamic> json) {
    return Directive(
      ...
      pipelineStock: (json['pipeline_stock'] as num?)?.toInt() ?? 0,
      reason: json['reason']?.toString() ?? '',
      priority: json['priority']?.toString() ?? 'low',
      recommendedQty: (json['recommended_qty'] as num?)?.toInt() ?? 0,
      ...
      budgetStatus: json['budget_status']?.toString() ?? 'funded',
      storeId: json['store_id']?.toString() ?? 'DS-BLR-INDIRANAGAR',
      directiveType: json['directive_type']?.toString() ?? 'purchase',
      transferSource: json['transfer_source']?.toString(),
      expiryRisk: json['expiry_risk']?.toString(),
      oldestBatchHours: (json['oldest_batch_hours'] as num?)?.toInt(),
      fulfillmentChannel: json['fulfillment_channel']?.toString() ?? 'bb_now',
    );
  }
